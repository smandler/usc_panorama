...
public class infoCubeManagerV2 : getReal3D.MonoBehaviourWithRpc {
    ...
    public slideStruct[] slidesTest;

	public Texture[] slides;
	public string[]  actions;
	public GameObject toggleOffParent;
	[System.Serializable]
	public struct toggleOnArray
	{
		public GameObject[] items;
	}
	public toggleOnArray[] toggleOn;
	public bool[] noSlideTransition;
	Texture slideToLoad;
	...
	void Update () {	
		...
		if(pushedInputChar != ""){
			...
			// if(pushedInputChar == "R" && slideChangeRequested == false){
				...
				if(currentSlide < slides.Length-1){
					...
					foreach(GameObject turnOn in toggleOn[currentSlide].items){ turnOn.SetActive (true); }
			...
			if(pushedInputChar == "S" && slideChangeRequested == false){
				...
				if(currentSlide > 0 && currentSlide < actions.Length){
					if(actions[currentSlide] != "")
					{
						Debug.LogWarning ("Tour requested\n");
						controllerGO.GetComponent<controller>().notify ("Infocube", actions[currentSlide], this.gameObject);
					}
				}
		...
		if(noSlideTransition[currentSlide] == false){
	...
	public void setSlideTextures_rpc(int argCurrentPanel, int argFrontSlideIndexToLoad, string argDirection){
		...
		for(int i=0; i<temp.Length; i++){
			...
			if(temp[i].name == targetSlideName){
				...
				temp[i].mainTexture = slides[argFrontSlideIndexToLoad];
			} else {
			...
			if(temp[i].name == targetSlideName){
				if(argFrontSlideIndexToLoad+1 > slides.Length-1)
				{
					...
					temp[i].mainTexture = slides[argFrontSlideIndexToLoad+1];
				}
			} else {
				// Debug.Log ("No, it is not");
			}
			...
			if(argDirection == "next"){
				...
				if(temp[i].name == targetSlideName){
					if(argFrontSlideIndexToLoad+2 > slides.Length-1)
					{
						...
						temp[i].mainTexture = slides[argFrontSlideIndexToLoad+2];
					}
				} else {
					// Debug.Log ("No, it is not");
				}
